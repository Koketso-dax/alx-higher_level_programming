#!/usr/bin/python3
"""Script to display values in the states table based
on user input (safe from MySQL injections)
"""
...
def search_state(username, password, database, state_name):
    """
    Function to search for a state in the states table
    based on user input (safe from MySQL injections).
...
if __name__ == "__main__":
    # Get MySQL usr, pawd, db name, and state name cmd args
    if len(sys.argv) != 5:
        print("Usage: ./3-my_safe_filter_states.py \
        <usr> <pwd> <db> <state_name>")
        sys.exit(1)
